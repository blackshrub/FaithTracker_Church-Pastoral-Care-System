          ...
          <div className="space-y-1 sm:space-y-2 min-w-0 flex-1">
            ...
            {change !== null && (
              <div className="flex items-center gap-1 text-xs sm:text-sm">
                ...
                <span
                  className={`truncate ${isPositive ? 'text-green-600' : isNeutral ? 'text-gray-500' : 'text-red-600'}`}
                >
                  {isPositive ? '+' : ''}
                  {change.toFixed(1)}
                  {format === 'percent' ? '%' : ''}{' '}
                  <span className="hidden xs:inline">vs last month</span>
                </span>
              </div>
            )}
          </div>
          <div
            className={`p-2 sm:p-3 rounded-full flex-shrink-0 ${statusColors[status] || 'bg-gray-100'}`}
          >
    ...
    <div className={`p-3 sm:p-4 rounded-lg ${bg} border ${border}`}>
      <div className="flex items-start gap-2 sm:gap-3">
        ...
        <div className="min-w-0 flex-1">
          <Badge variant="outline" className={`mb-1 text-xs ${text} border-current`}>
            {category}
          </Badge>
    ...
    <div className="p-4 bg-muted/30 rounded-lg">
      ...
      <div className="flex flex-col gap-3 sm:hidden">
        <div className="flex items-center gap-3">
          ...
          <Badge className={`${workloadColor[staff.workload_status]} text-xs`}>
            {staff.workload_status}
          </Badge>
        </div>
...
export const Reports = () => {
  ...
  const { data: monthlyReport, isLoading: loadingMonthly } = useQuery({
    ...
    queryFn: () =>
      api
        .get(`/reports/monthly?year=${selectedYear}&month=${selectedMonth}`)
        .then((res) => res.data),
  ...
  const { data: staffReport, isLoading: loadingStaff } = useQuery({
    ...
    queryFn: () =>
      api
        .get(`/reports/staff-performance?year=${selectedYear}&month=${selectedMonth}`)
        .then((res) => res.data),
    staleTime: 1000 * 60 * 5,
  });
  ...
  const fetchPDF = async () => {
    const response = await api.get(
      `/reports/monthly/pdf?year=${selectedYear}&month=${selectedMonth}`,
      {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 print:space-y-4 overflow-hidden" ref={reportRef}>
      ...
      <div className="flex flex-col gap-4 print:hidden">
        <div>
          ...
          <p className="text-sm sm:text-base text-muted-foreground mt-1">
            {t('reports.subtitle') || 'Management reports and staff performance analytics'}
          </p>
        </div>
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              disabled={exporting}
              className="flex-1 sm:flex-none bg-teal-600 hover:bg-teal-700 text-white disabled:opacity-50"
            >
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="print:hidden">
          ...
          <Card className="print:shadow-none print:border-2">
            <CardHeader className="pb-2">
              <CardTitle className="flex items-center gap-2">
                ...
                Executive Summary - {monthlyReport?.report_period?.month_name}{' '}
                {monthlyReport?.report_period?.year}
              </CardTitle>
              <CardDescription>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                ...
                <div className="p-3 sm:p-4 border rounded-lg">
                  ...
                  <p className="text-xl sm:text-2xl font-bold truncate">
                    Rp {(exec.financial_aid_total || 0).toLocaleString('id-ID')}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {exec.financial_aid_recipients || 0} recipients
                  </p>
                </div>
              </div>
            </CardContent>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold">
                      {ministry.grief_support?.families_supported || 0} families
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {ministry.grief_support?.total_touchpoints || 0} touchpoints
                    </p>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold">
                      {ministry.hospital_visits?.patients_visited || 0} patients
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {ministry.hospital_visits?.total_visits || 0} visits
                    </p>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold">
                      {ministry.birthday_ministry?.celebrated || 0} celebrated
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {ministry.birthday_ministry?.ignored
                        ? `${ministry.birthday_ministry.ignored} skipped · `
                        : ''}
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold">
                      Rp {(ministry.financial_aid?.total_amount || 0).toLocaleString('id-ID')}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {ministry.financial_aid?.recipients || 0} recipients
                    </p>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="p-4 sm:p-6">
                <div className="space-y-2 sm:space-y-3">
                  {monthlyReport?.care_breakdown?.map((care, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between gap-3 p-2 sm:p-3 bg-muted/30 rounded-lg"
                    >
                      <div className="min-w-0 flex-1">
              ...
              <CardContent className="p-4 sm:p-6">
                <div className="space-y-2 sm:space-y-3">
                  {monthlyReport?.insights?.length > 0 ? (
                    monthlyReport.insights.map((insight, i) => <InsightCard key={i} {...insight} />)
                  ) : (
                ...
                <div className="space-y-2 sm:space-y-3">
                  {monthlyReport?.recommendations?.length > 0 ? (
                    monthlyReport.recommendations.map((rec, i) => (
                      <div
                        key={i}
                        className="flex items-start gap-2 sm:gap-3 p-2 sm:p-3 bg-teal-50 dark:bg-teal-900/20 rounded-lg"
                      >
                  ...
                  <div key={key} className="p-2 sm:p-4 border rounded-lg text-center">
                    ...
                    <p className="text-lg sm:text-2xl font-bold truncate">
                      {typeof data.current === 'number' && data.current > 1000
                        ? data.current.toLocaleString('id-ID')
                        : data.current}
                    </p>
                    <div
                      className={`flex items-center justify-center gap-1 text-xs sm:text-sm ${data.change > 0 ? 'text-green-600' : data.change < 0 ? 'text-red-600' : 'text-gray-500'}`}
                    >
                      {data.change > 0 ? (
                        ...
                        <Minus className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" />
                      )}
                      <span className="truncate">
                        {data.change > 0 ? '+' : ''}
                        {typeof data.change === 'number' && Math.abs(data.change) > 1000
        ...
        <TabsContent value="staff" className="space-y-6 mt-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                Team Workload Overview - {staffReport?.report_period?.month_name}{' '}
                {staffReport?.report_period?.year}
              </CardTitle>
              <CardDescription>
                Monitor workload distribution to prevent burnout and ensure equitable task
                assignment
              </CardDescription>
            </CardHeader>
            <CardContent className="p-4 sm:p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                <div className="p-3 sm:p-4 border rounded-lg text-center">
                  ...
                  <p className="text-xl sm:text-2xl font-bold">
                    {staffReport?.team_stats?.total_tasks_completed || 0}
                  </p>
                </div>
                <div className="p-3 sm:p-4 border rounded-lg text-center">
                  <p className="text-xs sm:text-sm text-muted-foreground">Avg Tasks/Staff</p>
                  <p className="text-xl sm:text-2xl font-bold">
                    {staffReport?.team_stats?.average_tasks_per_staff || 0}
                  </p>
                </div>
                <div className="p-3 sm:p-4 border rounded-lg text-center">
                  <p className="text-xs sm:text-sm text-muted-foreground">Max - Min Difference</p>
                  <p className="text-xl sm:text-2xl font-bold">
                    {(staffReport?.team_stats?.max_tasks || 0) -
                      (staffReport?.team_stats?.min_tasks || 0)}
                  </p>
                </div>
              </div>
            </CardContent>
              ...
              <CardContent>
                <div className="space-y-3">
                  {staffReport.recommendations.map((rec, i) => (
                    <div
                      key={i}
                      className={`p-4 rounded-lg ${rec.priority === 'high' ? 'bg-red-100 dark:bg-red-900/30' : rec.priority === 'medium' ? 'bg-amber-100 dark:bg-amber-900/30' : 'bg-blue-100 dark:bg-blue-900/30'}`}
                    >
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {staffReport.top_performers.slice(0, 3).map((staff, i) => (
                    <div
                      key={staff.user_id}
                      className={`p-4 rounded-lg text-center ${i === 0 ? 'bg-amber-100 dark:bg-amber-900/30 border-2 border-amber-400' : 'bg-muted/30'}`}
                    >
                      <div
                        className={`w-16 h-16 mx-auto rounded-full flex items-center justify-center mb-3 ${i === 0 ? 'bg-amber-400 text-white' : i === 1 ? 'bg-gray-300 text-gray-700' : 'bg-orange-300 text-orange-800'}`}
                      >
                        {staff.photo_url ? (
                          <img
                            src={staff.photo_url}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Detailed breakdown of each staff member's activities
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                ...
                {(!staffReport?.staff_performance ||
                  staffReport.staff_performance.length === 0) && (
        ...
        <TabsContent value="yearly" className="space-y-4 sm:space-y-6 mt-4 sm:mt-6">
          <Card>
            <CardHeader className="pb-2 sm:pb-4">
              <CardTitle className="text-base sm:text-lg">
                Yearly Summary - {yearlyReport?.report_period?.year}
              </CardTitle>
            </CardHeader>
            <CardContent className="p-4 sm:p-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4">
                <div className="text-center p-3 sm:p-4 bg-teal-50 dark:bg-teal-900/20 rounded-lg">
                  <p className="text-2xl sm:text-3xl font-bold text-teal-700 dark:text-teal-400">
                    {yearlyReport?.yearly_totals?.total_members || 0}
                  </p>
                ...
                <div className="text-center p-3 sm:p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                  <p className="text-2xl sm:text-3xl font-bold text-blue-700 dark:text-blue-400">
                    {yearlyReport?.yearly_totals?.total_care_events || 0}
                  </p>
                  <p className="text-xs sm:text-sm text-muted-foreground">Care Events</p>
                </div>
                <div className="text-center p-3 sm:p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                  <p className="text-2xl sm:text-3xl font-bold text-green-700 dark:text-green-400">
                    {yearlyReport?.yearly_totals?.completion_rate || 0}%
                  </p>
                  <p className="text-xs sm:text-sm text-muted-foreground">Completion</p>
                </div>
                <div className="text-center p-3 sm:p-4 bg-amber-50 dark:bg-amber-900/20 rounded-lg">
                  <p className="text-lg sm:text-2xl font-bold text-amber-700 dark:text-amber-400 truncate">
                    Rp{' '}
                    {(yearlyReport?.yearly_totals?.total_financial_aid || 0).toLocaleString(
                      'id-ID'
                    )}
                  </p>
          ...
          <Card>
            <CardHeader className="pb-2 sm:pb-4">
              <CardTitle className="text-base sm:text-lg">
                Care Events by Type (Year Total)
              </CardTitle>
            </CardHeader>
            <CardContent className="p-4 sm:p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                {yearlyReport?.care_by_type?.map((care, i) => (
                  <div
                    key={i}
                    className="flex items-center justify-between gap-3 p-3 sm:p-4 bg-muted/30 rounded-lg"
                  >
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-sm sm:text-base truncate">{care.label}</p>
                      <p className="text-xs text-muted-foreground">
                        {care.completed} of {care.total} completed
                      </p>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <p className="text-xl sm:text-2xl font-bold">{care.total}</p>
                      <Progress
                        value={(care.completed / care.total) * 100 || 0}
      ...
      {/* Print-only content */}
      <div className="hidden print:block">{/* The content above will be printed */}</div>
    </div>
  );
};
