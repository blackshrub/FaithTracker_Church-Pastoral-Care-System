            ...
            {days.map((day, index) => {
              ...
              if (!day) {
                return (
                  <div
                    key={`empty-${index}`}
                    className="border rounded-lg p-2 min-h-[80px] bg-muted/20"
                  />
                );
              }
              ...
              return (
                <div
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') handleDateClick(day, dayEvents);
                  }}
                >
                  <div className={`text-sm font-semibold mb-1 ${isToday ? 'text-teal-700' : ''}`}>
                    {day.getDate()}
                  </div>
                  {dayEvents.slice(0, 2).map((e) => (
                    <div
                      key={e.id}
                      className="text-xs bg-teal-100 text-teal-700 rounded px-1 mb-1 truncate"
                    >
                ...
                <Card key={type} className="card-border-left-teal">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-lg flex items-center gap-2">
                      ...
                      {t(`event_types.${type}`) || type.replace('_', ' ').toUpperCase()} (
                      {typeEvents.length})
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      {typeEvents.map((event) => (
                        <div
                          key={event.id}
                          className={`p-4 ${config.bgColor} rounded-lg border ${event.completed ? 'opacity-60' : ''}`}
                        >
                                ...
                                onClick={async () => {
                                  try {
                                    ...
                                    if (type === 'grief_loss' && event.grief_stage_id) {
                                      await api.post(
                                        `/grief-support/${event.grief_stage_id}/complete`
                                      );
                                    } else {
