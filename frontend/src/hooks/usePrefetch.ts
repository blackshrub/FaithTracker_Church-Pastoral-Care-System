...
import type { Member, CareEvent, GriefStage, AccidentFollowup, FinancialAidSchedule } from '@/types';
...
export interface UsePrefetchReturn {
  prefetchMember: (memberId: string) => void;
  prefetchDashboard: () => void;
  prefetchMembersList: (page?: number, search?: string) => void;
  cancelPrefetch: () => void;
}
...
export function usePrefetch(): UsePrefetchReturn {
   ...
   */
  const prefetchMembersList = useCallback((page: number = 1, search: string = '') => {
