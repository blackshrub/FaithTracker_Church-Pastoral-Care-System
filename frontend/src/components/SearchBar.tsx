...
interface FlatResult {
  ...
  id: string;
  memberId?: string;
  data: SearchMember | SearchCareEvent;
}

const SearchBar = () => {
  const [query, setQuery] = useState('');
  const [results, setResults] = useState<SearchResults>({ members: [], care_events: [] });
  ...
  const allResults: FlatResult[] = [
    ...results.members.map((m) => ({ type: 'member' as const, id: m.id, data: m })),
    ...results.care_events.map((e) => ({
      type: 'event' as const,
      id: e.id,
                        ...
                        <img
                          src={
                            member.photo_url.startsWith('http')
                              ? member.photo_url
                              : `${import.meta.env.VITE_BACKEND_URL}${member.photo_url}`
                          }
                          alt=""
                    ...
                    <div className="flex-shrink-0">
                      ...
                      >
                        {member.engagement_status === 'at_risk'
                          ...
                          : member.engagement_status === 'active'
                            ? 'Active'
                            : member.engagement_status === 'disconnected'
                              ? 'Disconnected'
