  ...
  ({
    ...
    return (
      <article
        ...
        aria-label={`Task for ${event.member_name}${event.days_overdue != null && event.days_overdue > 0 ? `, ${event.days_overdue} days overdue` : ''}`}
      >
        {/* Overdue Badge - Top Right */}
        {event.days_overdue != null && event.days_overdue > 0 && (
          <span
            className={`absolute top-3 ${selectable ? 'right-10' : 'right-3'} px-2 py-1 bg-red-500 text-white text-xs font-semibold rounded shadow-sm z-10`}
            aria-hidden="true"
          >
