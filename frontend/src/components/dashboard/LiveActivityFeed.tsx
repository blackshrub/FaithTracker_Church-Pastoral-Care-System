...
interface StreamActivity extends ActivityLog {
  timestamp?: string;
}
...
const ActivityItem = ({ activity, isNew }: ActivityItemProps) => {
...
export function LiveActivityFeed({ maxItems = 10, className }: LiveActivityFeedProps) {
  ...
  const { isConnected, activities, error } = useActivityStream({
    ...
    onActivity: (activity: StreamActivity) => {
      ...
      <CardContent className="pt-0">
        {activities.length === 0 ? (
          ...
          <div className="space-y-2 max-h-[400px] overflow-y-auto">
            {activities.map((activity: StreamActivity) => (
              <ActivityItem
                key={activity.id}
                activity={activity}
