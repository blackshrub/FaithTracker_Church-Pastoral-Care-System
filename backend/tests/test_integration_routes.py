...
from enums import (
    ...
    UserRole, ActivityActionType, ScheduleFrequency
)
...
def make_mock_db():
    ...
    for coll in [
        'users', 'campuses', 'members', 'care_events', 'grief_support',
        'accident_followup', 'financial_aid_schedules', 'activity_logs',
...
def make_request(headers=None, client_ip="127.0.0.1"):
    ...
    req = MagicMock()
    default_headers = {"Authorization": f"Bearer test-token"}
    if headers:
...
def make_test_care_event(campus_id=TEST_CAMPUS_ID, member_id=TEST_MEMBER_ID,
                          event_id=None, event_type="birthday", completed=False):
    return {
...
class TestAuthRoutes:
    ...
    async def test_login_success_full_admin(self, mock_ip, mock_token, mock_clear,
        ...
        mock_db.campuses.find_one = AsyncMock(return_value={
            "id": TEST_CAMPUS_ID, "campus_name": "Main Campus"
        })
    ...
    async def test_list_users_full_admin(self, mock_admin):
        ...
        users_data = [{
            "id": TEST_ADMIN_ID, "email": "admin@test.com", "name": "Test Admin",
            "role": "full_admin", "campus_id": TEST_CAMPUS_ID, "phone": "+6281234567890",
            "is_active": True, "created_at": NOW, "campus_name": "Main Campus"
        }]
    ...
    async def test_register_user_success(self, mock_get_db):
        ...
        data = UserCreate(
            ...
            name="New User", phone="+6281234567894",
            role=UserRole.PASTOR, campus_id=TEST_CAMPUS_ID
        )
        req = make_request()
    ...
    async def test_register_user_duplicate_email(self, mock_get_db):
        ...
        data = UserCreate(
            ...
            name="Dup User", phone="+6281234567895",
            role=UserRole.PASTOR, campus_id=TEST_CAMPUS_ID
        )
        req = make_request()
        with pytest.raises(HTTPException) as exc_info:
    ...
    async def test_register_user_invalid_email(self, mock_get_db):
        ...
        data = UserCreate(
            ...
            name="Bad Email", phone="+6281234567895",
            role=UserRole.PASTOR, campus_id=TEST_CAMPUS_ID
        )
        req = make_request()
        with pytest.raises(HTTPException) as exc_info:
    ...
    async def test_register_non_admin_without_campus(self, mock_get_db):
        ...
        data = UserCreate(
            email="new@test.com", password="StrongPass123!",
            name="No Campus", phone="+6281234567896",
            role=UserRole.PASTOR, campus_id=None
        )
        req = make_request()
        with pytest.raises(HTTPException) as exc_info:
            await _fn(register_user)(data=data, request=req)
...
class TestCampusRoutes:
    ...
    async def test_list_campuses(self, mock_set_cache, mock_get_cache):
        ...
        campus_data = [{
            "id": TEST_CAMPUS_ID, "campus_name": "Main Campus",
            "is_active": True, "location": "Jakarta",
    ...
    async def test_update_campus_success(self, mock_user):
        ...
        mock_db.campuses.find_one = AsyncMock(return_value={
            "id": TEST_CAMPUS_ID, "campus_name": "Updated Campus"
        })
...
class TestCareEventRoutes:
    ...
    def setup(self):
        ...
        init_care_event_routes(
            ...
            generate_grief_timeline=MagicMock(return_value=[
                {"id": str(uuid.uuid4()), "stage": "1_week", "scheduled_date": "2026-04-05",
                 "member_id": TEST_MEMBER_ID, "care_event_id": "evt-1", "completed": False}
            ]),
            generate_accident_followup_timeline=MagicMock(return_value=[
                {"id": str(uuid.uuid4()), "stage": "first_followup", "scheduled_date": "2026-04-01",
                 "member_id": TEST_MEMBER_ID, "care_event_id": "evt-1", "campus_id": TEST_CAMPUS_ID,
                 "completed": False}
            ]),
    ...
    async def test_create_regular_contact_event(self, mock_user):
        ...
        data = CareEventCreate(
            member_id=TEST_MEMBER_ID, campus_id=TEST_CAMPUS_ID,
            event_type=EventType.REGULAR_CONTACT,
            event_date=TODAY, title="Regular Check-in"
        )
        req = make_request()
    ...
    async def test_create_grief_event_generates_timeline(self, mock_user):
        ...
        data = CareEventCreate(
            ...
            event_date=TODAY, title="Grief Support", grief_relationship="Father"
        )
        req = make_request()
        result = await _fn(create_care_event)(data=data, request=req)
    ...
    async def test_create_accident_event_generates_followup(self, mock_user):
        ...
        data = CareEventCreate(
            ...
            event_date=TODAY, title="Accident Event"
        )
        req = make_request()
        result = await _fn(create_care_event)(data=data, request=req)
    ...
    async def test_create_financial_aid_event(self, mock_user):
        ...
        data = CareEventCreate(
            ...
            event_date=TODAY, title="Financial Aid",
            aid_type=AidType.EMERGENCY, aid_amount=500000.0
        )
        req = make_request()
        result = await _fn(create_care_event)(data=data, request=req)
    ...
    async def test_create_financial_aid_missing_type(self, mock_user):
        ...
        data = CareEventCreate(
            member_id=TEST_MEMBER_ID, campus_id=TEST_CAMPUS_ID,
            event_type=EventType.FINANCIAL_AID,
            event_date=TODAY, title="Missing Aid Type",
            aid_type=None, aid_amount=500000.0
        )
        req = make_request()
        with pytest.raises(HTTPException) as exc_info:
    ...
    async def test_create_financial_aid_invalid_amount(self, mock_user):
        ...
        data = CareEventCreate(
            member_id=TEST_MEMBER_ID, campus_id=TEST_CAMPUS_ID,
            event_type=EventType.FINANCIAL_AID,
            event_date=TODAY, title="Bad Amount",
            aid_type=AidType.EMERGENCY, aid_amount=-100
        )
        req = make_request()
        with pytest.raises(HTTPException) as exc_info:
            await _fn(create_care_event)(data=data, request=req)
    ...
    async def test_create_birthday_event(self, mock_user):
        ...
        data = CareEventCreate(
            ...
            event_date=TODAY, title="Birthday"
        )
        req = make_request()
        result = await _fn(create_care_event)(data=data, request=req)
    ...
    async def test_create_childbirth_event_autocomplete(self, mock_user):
        ...
        data = CareEventCreate(
            ...
            event_date=TODAY, title="Childbirth"
        )
        req = make_request()
        result = await _fn(create_care_event)(data=data, request=req)
    ...
    async def test_create_event_pastor_enforces_campus(self, mock_user):
        ...
        data = CareEventCreate(
            member_id=TEST_MEMBER_ID, campus_id=TEST_CAMPUS_ID_2,
            event_type=EventType.REGULAR_CONTACT,
            event_date=TODAY, title="Test"
        )
        req = make_request()
        result = await _fn(create_care_event)(data=data, request=req)
        assert result.campus_id == pastor["campus_id"]
    ...
    async def test_list_care_events_with_filters(self, mock_user):
        ...
        result = await _fn(list_care_events)(
            request=req, event_type=EventType.BIRTHDAY,
            member_id=TEST_MEMBER_ID, completed=False, page=2, limit=10
        )
        assert result == []
    ...
    async def test_bulk_complete_success(self, mock_user):
        from routes.care_events import bulk_complete_care_events, BulkEventIds
    ...
    async def test_bulk_ignore_success(self, mock_user):
        from routes.care_events import bulk_ignore_care_events, BulkEventIds
    ...
    async def test_bulk_delete_success(self, mock_user):
        from routes.care_events import bulk_delete_care_events, BulkEventIds
...
class TestGriefSupportRoutes:
    ...
    async def test_list_grief_support(self, mock_user):
        ...
        mock_user.return_value = make_admin_user()
        stages = [{"id": str(uuid.uuid4()), "stage": "1_week", "completed": False,
                    "member_id": TEST_MEMBER_ID, "scheduled_date": TODAY.isoformat()}]
    ...
    async def test_complete_grief_stage_success(self, mock_user):
        ...
        stage = {"id": "stage-1", "stage": "1_week", "member_id": TEST_MEMBER_ID,
                 "campus_id": TEST_CAMPUS_ID, "care_event_id": "evt-1"}
...
class TestAccidentFollowupRoutes:
    ...
    async def test_list_accident_followup(self, mock_user):
        ...
        mock_user.return_value = make_admin_user()
        stages = [{"id": "f1", "stage": "first_followup", "completed": False,
                    "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID}]
    ...
    async def test_complete_accident_stage_success(self, mock_user):
        ...
        mock_user.return_value = make_admin_user()
        stage = {"id": "f1", "stage": "first_followup", "member_id": TEST_MEMBER_ID,
                 "campus_id": TEST_CAMPUS_ID, "care_event_id": "evt-1"}
...
class TestFinancialAidRoutes:
    ...
    async def test_create_aid_schedule_monthly(self, mock_user):
        ...
        data = FinancialAidScheduleCreate(
            member_id=TEST_MEMBER_ID, title="Monthly Aid",
            aid_type="education", aid_amount=500000,
    ...
    async def test_create_aid_schedule_weekly(self, mock_user):
        ...
        data = FinancialAidScheduleCreate(
            member_id=TEST_MEMBER_ID, title="Weekly Aid",
            aid_type="food", aid_amount=100000,
    ...
    async def test_create_aid_schedule_annually(self, mock_user):
        ...
        data = FinancialAidScheduleCreate(
            member_id=TEST_MEMBER_ID, title="Annual Aid",
            aid_type="education", aid_amount=5000000,
            frequency="annually", start_date=TODAY.isoformat(),
    ...
    async def test_list_aid_schedules_with_filters(self, mock_user):
        ...
        req = make_request()
        result = await _fn(list_aid_schedules)(request=req, member_id=TEST_MEMBER_ID, active_only=False, page=1, limit=50)
        assert result == []
    ...
    async def test_stop_aid_schedule_success(self, mock_user):
        ...
        mock_user.return_value = make_admin_user()
        schedule = {"id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
                     "aid_type": "education", "is_active": True}
    ...
    async def test_mark_aid_distributed_weekly(self, mock_user):
        ...
        mock_user.return_value = make_admin_user()
        schedule = {
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "food", "aid_amount": 100000, "frequency": "weekly",
            "title": "Weekly Food", "next_occurrence": TODAY.isoformat(),
            "occurrences_completed": 2, "is_active": True
        }
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_mark_aid_distributed_monthly(self, mock_user):
        ...
        schedule = {
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "education", "aid_amount": 500000, "frequency": "monthly",
            "title": "Monthly Education", "next_occurrence": "2026-03-15",
            "day_of_month": 15, "occurrences_completed": 0, "is_active": True
        }
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_ignore_financial_aid_schedule_success(self, mock_user):
        ...
        schedule = {
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "food", "frequency": "weekly", "next_occurrence": TODAY.isoformat(),
            "ignored_occurrences": [], "is_active": True
        }
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_ignore_financial_aid_no_next(self, mock_user):
        ...
        mock_user.return_value = make_admin_user()
        schedule = {"id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
                     "frequency": "weekly", "next_occurrence": None, "ignored_occurrences": []}
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_remove_ignored_occurrence(self, mock_user):
        ...
        req = make_request()
        result = await _fn(remove_ignored_occurrence)(
            schedule_id="s1", occurrence_date="2026-03-01", request=req
        )
        assert result["success"] is True
    ...
    async def test_remove_ignored_occurrence_not_found(self, mock_user):
        ...
        req = make_request()
        with pytest.raises(HTTPException) as exc_info:
            await _fn(remove_ignored_occurrence)(
                schedule_id="nonexistent", occurrence_date="2026-03-01", request=req
            )
        assert exc_info.value.status_code == 404
    ...
    async def test_clear_all_ignored_occurrences(self, mock_user):
        ...
        schedule = {
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "food", "ignored_occurrences": ["2026-03-01"]
        }
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_get_aid_due_today(self, mock_user):
        ...
        schedules = [{
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "food", "aid_amount": 100000, "next_occurrence": TODAY.isoformat(),
            "is_active": True
        }]
        mock_db.financial_aid_schedules.find = MagicMock(return_value=make_cursor(schedules))
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_get_financial_aid_recipients(self):
        ...
        agg_data = [
            {"_id": TEST_MEMBER_ID, "total_amount": 600000, "aid_count": 3}
        ]
    ...
    async def test_get_financial_aid_recipients_no_member(self):
        ...
        agg_data = [
            {"_id": TEST_MEMBER_ID, "total_amount": 100000, "aid_count": 1}
        ]
        mock_db.care_events.aggregate = MagicMock(return_value=make_agg_cursor(agg_data))
        mock_db.members.find_one = AsyncMock(return_value=None)
...
class TestDashboardRoutes:
    ...
    def setup(self):
        ...
        init_dashboard_routes(
            get_campus_timezone=AsyncMock(return_value="Asia/Jakarta"),
            get_date_in_timezone=MagicMock(return_value=TODAY.isoformat()),
            get_writeoff_settings=AsyncMock(return_value={
                "birthday": 7, "grief_support": 30, "accident_illness": 14, "financial_aid": 30
            }),
        )
    ...
    async def test_get_dashboard_stats(self, mock_cache, mock_user):
        ...
        mock_user.return_value = make_admin_user()
        mock_db.members.aggregate = MagicMock(return_value=make_agg_cursor([{
            "total_count": [{"count": 100}],
            "at_risk_count": [{"count": 10}]
        }]))
...
class TestDashboardHelpers:
    ...
    async def test_calculate_reminders_with_birthday_today(self):
        ...
        result = await _fn(calculate_dashboard_reminders)(
            TEST_CAMPUS_ID, "Asia/Jakarta", TODAY.isoformat()
        )
    ...
    async def test_calculate_reminders_with_grief_today(self):
        ...
        member = make_test_member()
        grief_stage = {
            "id": "g1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "stage": "1_week", "scheduled_date": TODAY.isoformat(),
    ...
    async def test_calculate_reminders_with_accident_overdue(self):
        ...
        accident_followup = {
            "id": "a1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "stage": "first_followup", "scheduled_date": yesterday,
            "completed": False, "ignored": False
        }
        mock_db.members.find = MagicMock(return_value=make_cursor([member]))
        mock_db.grief_support.find = MagicMock(return_value=make_cursor([]))
    ...
    async def test_calculate_reminders_with_financial_aid_due(self):
        ...
        member = make_test_member()
        aid_schedule = {
            "id": "f1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_amount": 100000, "frequency": "monthly",
            "next_occurrence": TODAY.isoformat(), "is_active": True
        }
        mock_db.members.find = MagicMock(return_value=make_cursor([member]))
        mock_db.grief_support.find = MagicMock(return_value=make_cursor([]))
        mock_db.accident_followup.find = MagicMock(return_value=make_cursor([]))
    ...
    async def test_calculate_reminders_upcoming_tasks(self):
        ...
        grief_stage = {
            "id": "g1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "stage": "2_weeks", "scheduled_date": future_date,
            "completed": False, "ignored": False
        }
        mock_db.members.find = MagicMock(return_value=make_cursor([member]))
        mock_db.grief_support.find = MagicMock(return_value=make_cursor([grief_stage]))
    ...
    async def test_calculate_reminders_completed_birthday(self):
        ...
        birthday_event = {
            "member_id": TEST_MEMBER_ID, "completed": True,
            "completed_at": completed_at_naive, "completed_by_user_name": "Pastor John",
            "ignored": False, "ignored_at": None
        }
        mock_db.members.find = MagicMock(return_value=make_cursor([member]))
        mock_db.grief_support.find = MagicMock(return_value=make_cursor([]))
        mock_db.accident_followup.find = MagicMock(return_value=make_cursor([]))
...
class TestDependencies:
    ...
    async def test_get_current_admin_not_admin(self):
        from dependencies import get_current_admin, create_access_token
        from litestar.exceptions import HTTPException
    ...
    async def test_get_full_admin_not_full_admin(self):
        from dependencies import get_full_admin, create_access_token
        from litestar.exceptions import HTTPException
