...
class TestAuthRoutesCoverage:
    ...
    async def test_register_user_weak_password(self, mock_get_db):
        ...
        data = UserCreate(
            ...
            name="Weak Pass User", phone="+6281234567894",
    ...
    async def test_register_user_db_error(self, mock_get_db):
        ...
        data = UserCreate(
            ...
            name="Error User", phone="+6281234567894",
            role=UserRole.PASTOR, campus_id=TEST_CAMPUS_ID
        )
        req = make_request()
        with pytest.raises(HTTPException) as exc_info:
...
class TestFinancialAidRoutesCoverage:
    ...
    async def test_create_weekly_schedule_next_week(self, mock_user):
        ...
        data = FinancialAidScheduleCreate(
            member_id=TEST_MEMBER_ID, title="Weekly Sunday Aid",
    ...
    async def test_create_monthly_schedule_invalid_day(self, mock_user):
        ...
        data = FinancialAidScheduleCreate(
            member_id=TEST_MEMBER_ID, title="Monthly Bad Day",
    ...
    async def test_create_annual_schedule_feb_29_non_leap(self, mock_user):
        ...
        data = FinancialAidScheduleCreate(
            member_id=TEST_MEMBER_ID, title="Feb 29 Aid",
            aid_type="education", aid_amount=1000000,
            frequency="annually", start_date=TODAY.isoformat(),
            month_of_year=2, day_of_month=30  # Feb 30 doesn't exist
        )
        req = make_request()
        result = await _fn(create_aid_schedule)(data=data, request=req)
        assert result is not None
    ...
    async def test_create_annual_schedule_apr_31(self, mock_user):
        ...
        data = FinancialAidScheduleCreate(
            member_id=TEST_MEMBER_ID, title="Apr 31 Aid",
            aid_type="education", aid_amount=1000000,
            frequency="annually", start_date=TODAY.isoformat(),
            month_of_year=4, day_of_month=31  # Apr has 30 days
        )
        req = make_request()
        result = await _fn(create_aid_schedule)(data=data, request=req)
        assert result is not None
    ...
    async def test_create_annual_schedule_31day_month(self, mock_user):
        ...
        data = FinancialAidScheduleCreate(
            member_id=TEST_MEMBER_ID, title="Jul Aid",
            aid_type="education", aid_amount=1000000,
            frequency="annually", start_date=TODAY.isoformat(),
            month_of_year=7, day_of_month=15
        )
        req = make_request()
        result = await _fn(create_aid_schedule)(data=data, request=req)
        assert result is not None
    ...
    async def test_create_aid_schedule_db_error(self, mock_user):
        ...
        data = FinancialAidScheduleCreate(
            member_id=TEST_MEMBER_ID, title="Error Aid",
            aid_type="food", aid_amount=100000,
            frequency="weekly", start_date=TODAY.isoformat(),
            day_of_week="monday"
        )
    ...
    async def test_mark_distributed_monthly_december(self, mock_user):
        ...
        schedule = {
            ...
            "title": "Monthly Education", "next_occurrence": "2026-12-15",
            "day_of_month": 15, "occurrences_completed": 5, "is_active": True
        }
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_mark_distributed_monthly_day_overflow_feb(self, mock_user):
        ...
        schedule = {
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "food", "aid_amount": 100000, "frequency": "monthly",
            "title": "Monthly Food", "next_occurrence": "2026-01-31",
            "day_of_month": 31, "occurrences_completed": 0, "is_active": True
        }
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_mark_distributed_monthly_day_overflow_30day(self, mock_user):
        ...
        schedule = {
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "food", "aid_amount": 100000, "frequency": "monthly",
            "title": "Monthly Food", "next_occurrence": "2026-03-31",
            "day_of_month": 31, "occurrences_completed": 1, "is_active": True
        }
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_mark_distributed_annually(self, mock_user):
        ...
        schedule = {
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "education", "aid_amount": 5000000, "frequency": "annually",
            "title": "Annual Aid", "next_occurrence": "2026-06-01",
            "occurrences_completed": 0, "is_active": True
        }
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_mark_distributed_unknown_frequency(self, mock_user):
        ...
        schedule = {
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "food", "aid_amount": 100000, "frequency": "one_time",
            "title": "One Time Aid", "next_occurrence": TODAY.isoformat(),
            "occurrences_completed": 0, "is_active": True
        }
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_ignore_aid_monthly(self, mock_user):
        ...
        schedule = {
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "food", "frequency": "monthly", "next_occurrence": "2026-03-15",
            "day_of_month": 15, "ignored_occurrences": [], "is_active": True
        }
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_ignore_aid_monthly_december(self, mock_user):
        ...
        schedule = {
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "food", "frequency": "monthly", "next_occurrence": "2026-12-15",
            "day_of_month": 15, "ignored_occurrences": [], "is_active": True
        }
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_ignore_aid_monthly_day_overflow_feb(self, mock_user):
        ...
        schedule = {
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "food", "frequency": "monthly", "next_occurrence": "2026-01-31",
            "day_of_month": 31, "ignored_occurrences": [], "is_active": True
        }
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_ignore_aid_monthly_day_overflow_30day(self, mock_user):
        ...
        schedule = {
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "food", "frequency": "monthly", "next_occurrence": "2026-03-31",
            "day_of_month": 31, "ignored_occurrences": [], "is_active": True
        }
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_ignore_aid_annually(self, mock_user):
        ...
        schedule = {
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "education", "frequency": "annually", "next_occurrence": "2026-06-01",
    ...
    async def test_ignore_aid_unknown_frequency(self, mock_user):
        ...
        schedule = {
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "food", "frequency": "one_time", "next_occurrence": TODAY.isoformat(),
            "ignored_occurrences": [], "is_active": True
        }
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_get_financial_aid_recipients_name_from_title(self):
        ...
        mock_db.care_events.find_one = AsyncMock(return_value={
            "title": "Financial Aid - Jane Doe"
        })
    ...
    async def test_get_financial_aid_recipients_null_member_id(self):
        ...
        agg_data = [
            {"_id": None, "total_amount": 50000, "aid_count": 1}
        ]
        mock_db.care_events.aggregate = MagicMock(return_value=make_agg_cursor(agg_data))
    ...
    async def test_ignore_aid_duplicate_occurrence(self, mock_user):
        ...
        schedule = {
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "food", "frequency": "weekly", "next_occurrence": occurrence,
            "ignored_occurrences": [occurrence], "is_active": True
        }
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_get_aid_due_today_overdue(self, mock_user):
        ...
        schedules = [{
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "food", "aid_amount": 100000, "next_occurrence": past_date,
            "is_active": True
        }]
        mock_db.financial_aid_schedules.find = MagicMock(return_value=make_cursor(schedules))
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
    ...
    async def test_mark_distributed_monthly_may31_overflow(self, mock_user):
        ...
        schedule = {
            "id": "s1", "member_id": TEST_MEMBER_ID, "campus_id": TEST_CAMPUS_ID,
            "aid_type": "food", "aid_amount": 100000, "frequency": "monthly",
            "title": "Monthly Food", "next_occurrence": "2026-05-31",
            "day_of_month": 31, "occurrences_completed": 2, "is_active": True
        }
        mock_db.financial_aid_schedules.find_one = AsyncMock(return_value=schedule)
        mock_db.members.find_one = AsyncMock(return_value=make_test_member())
