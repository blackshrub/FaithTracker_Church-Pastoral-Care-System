...
from enums import ActivityActionType, EventType, UserRole
from models import (
    AdditionalVisitRequest,
    CareEvent,
    CareEventCreate,
    CareEventUpdate,
    VisitationLogEntry,
...
_generate_grief_timeline: Callable[[date, str, str], list[dict[str, Any]]] | None = None
_generate_accident_followup_timeline: Callable[[date, str, str, str], list[dict[str, Any]]] | None = None
...
async def create_care_event(data: CareEventCreate, request: Request) -> dict:
    ...
    try:
        ...
        if event.event_type == EventType.FINANCIAL_AID:
            logger.info(f"[FINANCIAL AID] Creating event: aid_type={event.aid_type!r}, aid_amount={event.aid_amount!r}")
            if not event.aid_type:
                logger.warning(f"[FINANCIAL AID] Rejecting: aid_type is missing or falsy: {event.aid_type!r}")
            ...
            if event.aid_amount is None or event.aid_amount < 0:
                logger.warning(f"[FINANCIAL AID] Rejecting: aid_amount is invalid: {event.aid_amount!r}")
                raise HTTPException(
        ...
        care_event = CareEvent(
            ...
            completed_at=datetime.now(UTC) if is_one_time else None,
        ...
        if event.event_type == EventType.FINANCIAL_AID:
            logger.info(
                f"[FINANCIAL AID] Saving to DB: aid_type={event_dict.get('aid_type')!r}, aid_amount={event_dict.get('aid_amount')!r}"
            )
        ...
        if event.event_type == EventType.ACCIDENT_ILLNESS and _generate_accident_followup_timeline:
            timeline = _generate_accident_followup_timeline(event.event_date, care_event.id, event.member_id, campus_id)
            if timeline:
        ...
        logger.error(f"Error creating care event: {e!s}")
...
async def get_care_event(event_id: str, request: Request) -> dict:
    ...
    try:
        ...
        logger.error(f"Error getting care event: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
...
async def update_care_event(event_id: str, data: CareEventUpdate, request: Request) -> dict:
    ...
    try:
        ...
        result = await db.care_events.update_one({"id": event_id}, {"$set": update_data})
        ...
        logger.error(f"Error updating care event: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
...
async def complete_birthday_by_member(member_id: str, request: Request) -> dict:
    ...
    try:
        ...
        logger.error(f"Error completing birthday by member: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))


@post("/care-events/birthday/member/{member_id:str}/ignore")
async def ignore_birthday_by_member(member_id: str, request: Request) -> dict:
    """
    Ignore a birthday for a member by member_id.
    Creates the birthday care_event if it doesn't exist (some members have
    a birth_date but no pre-existing birthday care_event row — those are
    computed at dashboard-render time). Mirrors complete_birthday_by_member
    so the Dashboard "Ignore" button works for both real and computed
    birthday tasks. Without this endpoint the frontend was POSTing to
    /care-events/undefined/ignore and 404ing silently.
    """
    current_user = await get_current_user(request)
    db = get_db()
    try:
        ...
        if _get_campus_timezone:
            campus_tz = await _get_campus_timezone(campus_id)
        today_date = (
            _get_date_in_timezone(campus_tz) if _get_date_in_timezone else date.today().isoformat()
        )
        ...
        if existing_event:
            if existing_event.get("ignored"):
                return {"success": True, "message": "Birthday already ignored"}
            event_id = existing_event["id"]
            await db.care_events.update_one(
                {"id": event_id},
                {
                    "$set": {
                        ...
                        "ignored_by_name": current_user["name"],
                        "updated_at": now,
                    }
                },
            )
            ...
            await db.care_events.insert_one(
                {
                    ...
                    "event_date": today_date,
                    "title": f"Birthday - {member['name']}",
                    "description": f"Birthday for {member['name']}",
                    "completed": False,
                    "ignored": True,
                    "ignored_at": now,
                    "ignored_by": current_user["id"],
        ...
        return {"success": True, "message": "Birthday ignored successfully"}

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error ignoring birthday by member: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
...
async def complete_care_event(event_id: str, request: Request) -> dict:
    ...
    try:
        ...
        logger.error(f"Error completing care event: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
...
async def log_additional_visit(
    ...
    try:
        ...
        await db.members.update_one(
            ...
            {
                "$set": {
                    "last_contact_date": datetime.now(UTC),
                    "engagement_status": "active",
                    "days_since_last_contact": 0,
                }
            },
        ...
        logger.error(f"Error logging additional visit: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
...
async def send_care_event_reminder(event_id: str, request: Request) -> dict:
    ...
    try:
        ...
        result = await _send_whatsapp_message(
            member["phone"], message, care_event_id=event_id, member_id=event["member_id"]
        )
        ...
        logger.error(f"Error sending reminder: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
...
async def add_visitation_log(event_id: str, entry: VisitationLogEntry, request: Request) -> dict:
    ...
    try:
        ...
        logger.error(f"Error adding visitation log: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
...
async def bulk_complete_care_events(request: Request, data: BulkEventIds) -> dict:
    ...
    try:
        ...
        logger.error(f"Error in bulk complete: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
...
async def bulk_ignore_care_events(request: Request, data: BulkEventIds) -> dict:
    ...
    try:
        ...
        logger.error(f"Error in bulk ignore: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
...
route_handlers = [
    ...
    ignore_birthday_by_member,  # Must come before ignore_care_event (in server.py) for route priority
    complete_care_event,
