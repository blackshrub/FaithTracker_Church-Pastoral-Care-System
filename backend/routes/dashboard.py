...
_get_writeoff_settings: Callable[[], Awaitable[dict]] | None = None
...
async def calculate_dashboard_reminders(campus_id: str, campus_tz, today_date: str):
    ...
    try:
        ...
        birthday_members_task = db.members.find(
            {"campus_id": campus_id, "is_archived": {"$ne": True}, "birth_date": {"$regex": birthday_regex}},
            {
                "_id": 0,
                "id": 1,
                "name": 1,
        ...
        (
            ...
            writeoff_task,
            birthday_members_task,
            members_task,
            grief_task,
            accident_task,
            aid_task,
            birthday_events_task,
        )
                ...
                today_tasks.append(
                    {
                        ...
                        "days_since_last_contact": member_map.get(followup["member_id"], {}).get(
        ...
        at_risk = [
            ...
            for m in members
            if m.get("engagement_status") == "at_risk"
        ]
        disconnected = [
            {
                "type": "disconnected",
            ...
            for m in members
            if m.get("engagement_status") == "disconnected"
        ]
                    ...
                    today_tasks.append(
                        {
                            ...
                            "days_since_last_contact": member_map.get(schedule["member_id"], {}).get(
                                "days_since_last_contact"
                            ),
        ...
        for bday_member in birthday_members:
            ...
            try:
                ...
                base_data = {
                    ...
                    "details": f"Turning {member.get('age', '?')} years old",
                    "data": member,
                ...
                if this_year_birthday == today:
                    ...
                    if (
                        not is_completed
                        and not is_ignored
                        and (birthday_writeoff == 0 or days_overdue <= birthday_writeoff)
                    ):
                        overdue_birthdays.append(
                            {**base_data, "date": this_year_birthday.isoformat(), "days_overdue": days_overdue}
                        )
                ...
                today_tasks.append(
                    {
                        ...
                        "days_since_last_contact": member_map.get(stage["member_id"], {}).get(
                            "days_since_last_contact"
                        ),
        ...
        return {
            ...
            "total_tasks": len(birthdays_today)
            + len(grief_today)
            + len(accident_today)
            + len(at_risk)
            + len(disconnected),
        ...
        logger.error(f"Error calculating dashboard reminders: {e!s}")
        raise
...
async def get_dashboard_reminders(request: Request) -> dict:
    ...
    try:
        ...
        if cache:
            ...
            try:
                lock_acquired = await cache._client.set(cache._make_key(lock_key, campus_id), "1", nx=True, ex=30)
        ...
        logger.error(f"Error getting dashboard reminders: {e!s}")
...
async def get_engagement_trends(request: Request, days: int = 30) -> dict:
    ...
    try:
        ...
        logger.error(f"Error getting engagement trends: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
...
async def get_grief_completion_rate(request: Request) -> dict:
    ...
    try:
        ...
        logger.error(f"Error getting grief completion rate: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))


@get("/analytics/dashboard")
async def get_analytics_dashboard(
    request: Request, time_range: str = "all", start_date: str | None = None, end_date: str | None = None
) -> dict:
    ...
    try:
        ...
        total_members, members_with_photos, grief_total, grief_completed = await asyncio.gather(
            ...
            db.members.count_documents({**member_filter, "photo_url": {"$exists": True, "$nin": [None, ""]}}),
        ...
        events_by_type = [
            {
                "name": (e["_id"] or "unknown").replace("_", " ").upper(),
                "value": e["count"],
        ...
        financial_by_type = [
            {"name": (f["_id"] or "other").replace("_", " "), "value": f["total_amount"], "count": f["count"]}
        ...
        logger.error(f"Error getting analytics dashboard: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
    ...
    try:
        ...
        for member in members:
            ...
            age_group = (
                "Children (0-12)"
                if age <= 12
                else "Teenagers (13-17)"
                if age <= 17
                else "Young Adults (18-30)"
                if age <= 30
                else "Adults (31-60)"
                if age <= 60
                else "Seniors (60+)"
