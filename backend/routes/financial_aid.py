...
_get_engagement_settings_cached: Callable[[], Awaitable[dict]] | None = None
    ...
    try:
        ...
        if schedule["frequency"] == "weekly" and schedule.get("day_of_week"):
            ...
            try:
                ...
                raise HTTPException(status_code=400, detail=f"Day {day_of_month} does not exist in the specified month")
        ...
        logger.error(f"Error creating aid schedule: {e!s}")
...
async def remove_ignored_occurrence(schedule_id: str, occurrence_date: str, request: Request) -> dict:
    ...
    try:
        ...
        logger.error(f"Error removing ignored occurrence: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
...
async def clear_all_ignored_occurrences(schedule_id: str, request: Request) -> dict:
    ...
    try:
        ...
        await db.financial_aid_schedules.update_one(
            {"id": schedule_id}, {"$set": {"ignored_occurrences": [], "updated_at": datetime.now(UTC)}}
        )
        ...
        logger.error(f"Error clearing ignored occurrences: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
...
async def delete_aid_schedule(schedule_id: str, request: Request) -> dict:
    ...
    try:
        ...
        logger.error(f"Error deleting aid schedule: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
...
async def stop_aid_schedule(schedule_id: str, request: Request) -> dict:
    ...
    try:
        ...
        logger.error(f"Error stopping aid schedule: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
    ...
    try:
        ...
        if len(filtered) == 0 and len(schedules) > 0:
            ...
            for s in schedules[:3]:  # Check first 3
                logger.info(
                    f"  Debug schedule: id={s.get('id')[:8]}, is_active={s.get('is_active')} (type={type(s.get('is_active'))}), ignored_occ={s.get('ignored_occurrences')} (type={type(s.get('ignored_occurrences'))}), member_id={s.get('member_id')[:8] if s.get('member_id') else 'None'}"
                )
    ...
    try:
        ...
        for schedule in schedules:
            member = await db.members.find_one(
                {"id": schedule["member_id"]}, {"_id": 0, "name": 1, "phone": 1, "photo_url": 1}
            )
            if member:
        ...
        logger.error(f"Error getting aid due today: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
...
async def mark_aid_distributed(schedule_id: str, request: Request) -> dict:
    ...
    try:
        ...
        status, days = calculate_engagement_status(
            datetime.now(UTC), settings.get("atRiskDays", 60), settings.get("disconnectedDays", 90)
        )
        ...
        logger.info(
            f"[DISTRIBUTE] Before update - Schedule {schedule_id}: is_active={schedule.get('is_active')}, ignored_occurrences={schedule.get('ignored_occurrences')}, next_occurrence={schedule.get('next_occurrence')}"
        )
        ...
        logger.info(
            f"[DISTRIBUTE] After update - Schedule {schedule_id}: is_active={updated_schedule.get('is_active')}, ignored_occurrences={updated_schedule.get('ignored_occurrences')}, next_occurrence={updated_schedule.get('next_occurrence')}"
        )
        ...
        logger.error(f"Error marking aid distributed: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
...
async def ignore_financial_aid_schedule(schedule_id: str, request: Request) -> dict:
    ...
    try:
        ...
        current_date = (
            date.fromisoformat(current_occurrence) if isinstance(current_occurrence, str) else current_occurrence
        )

        # Log before update for debugging
        logger.info(
            f"[IGNORE] Before update - Schedule {schedule_id}: member_id={schedule.get('member_id')}, is_active={schedule.get('is_active')}, ignored_occurrences={schedule.get('ignored_occurrences')}, next_occurrence={schedule.get('next_occurrence')}"
        )
