...
from dependencies import get_campus_filter, get_current_user, get_db, logger, safe_error_detail
from enums import ActivityActionType, EventType
from models import generate_uuid

# Callbacks to be injected from server.py
_invalidate_dashboard_cache: Callable | None = None
_log_activity: Callable | None = None
_get_campus_timezone: Callable | None = None
_get_date_in_timezone: Callable | None = None
...
async def list_accident_followup(
    ...
    try:
        ...
        skip = (page - 1) * limit
        stages = (
            await db.accident_followup.find(query, {"_id": 0})
        ...
        logger.error(f"Error listing accident follow-up: {e!s}")
...
async def get_member_accident_timeline(member_id: str, request: Request) -> dict:
    ...
    try:
        ...
        timeline = await db.accident_followup.find(query, {"_id": 0}).sort("scheduled_date", 1).to_list(100)

        return timeline
    except Exception as e:
        logger.error(f"Error getting member accident timeline: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))


@post("/accident-followup/{stage_id:str}/complete")
async def complete_accident_stage(stage_id: str, request: Request, notes: str | None = None) -> dict:
    ...
    try:
        ...
        result = await db.accident_followup.update_one({"id": stage_id}, {"$set": update_data})
        ...
        logger.error(f"Error completing accident stage: {e!s}")
        raise HTTPException(status_code=500, detail=safe_error_detail(e))
